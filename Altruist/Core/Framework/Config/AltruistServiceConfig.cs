            ...
            foreach (var svcAttr in implType.GetCustomAttributes<ServiceAttribute>())
            {
                ...
                foreach (var itemSection in items)
                {
                    ...
                    Func<IServiceProvider, object> factory =
                        sp => DependencyResolver.CreateWithConfiguration(sp, itemCfg, implType, log, lifetime);

                    // Register unkeyed instances so IEnumerable<T> resolves all list items.
                    services.Add(new ServiceDescriptor(serviceType, sp => factory(sp), lifetime));
                    if (serviceType != implType)
                        services.Add(new ServiceDescriptor(implType, sp => factory(sp), lifetime));
                    ...
                    reg.Add($"\t{DependencyResolver.GetCleanName(serviceType)} + {DependencyResolver.GetCleanName(implType)} ({lifetime}) [list item]");
                    if (serviceType != implType)
                        reg.Add($"\t{DependencyResolver.GetCleanName(implType)} + {DependencyResolver.GetCleanName(implType)} ({lifetime}) [list item]");
                }
            }
        }
